...
export class PixiLayerRapid extends AbstractLayer {
  ...
  set enabled(val) {
    ...
    if (val) {
      ...
      this.context.services.geonode.startAsync();
    }
  }
  ...
  renderDataset(dataset, frame, projection, zoom) {
    ...
    if (dataset.service === 'mapwithai') {
      ...
      /* ESRI ArcGIS or GeoNode*/
    } else if (dataset.service === 'esri' || dataset.service === 'geonode') {
      if (zoom >= 14 && dataset.service === 'esri') {  // avoid firing off too many API requests
        service.loadTiles(datasetID);  // fetch more
      }
